 ...
 */
export type DeclarationConfigUpdate = Pick<
  DeclarationConfig,
  "visible"
>;
...
/**
 * Internal events emitted by Declaration.
 */
export type DeclarationEvents = {
 ...
 *
 * @typeParam Config - Configuration type for the layer (extends DeclarationConfig)
 * @typeParam UpdateConfig - Configuration properties that can be updated (extends DeclarationConfigUpdate)
 ...
 * class MyCustomMeshLayer extends Declaration<MyConfig, MyUpdateConfig, Mesh> {
 *   onCreate() {
...
export abstract class Declaration<
  Config extends DeclarationConfig = DeclarationConfig,
  UpdateConfig extends DeclarationConfigUpdate =
    DeclarationConfigUpdate,
  Instance extends BaseInstance = BaseInstance,
  CustomEvent extends BaseEventMap = BaseEventMap,
> extends EventHandler<DeclarationEvents & CustomEvent> {
  ...
  /**
   * Called when the layer configuration is updated via `Handle.update()`.
  ...
  /**
   * Called when the layer is deleted via `Handle.delete()`.
