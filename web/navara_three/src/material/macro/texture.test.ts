...
test("generateMixOverlaidTexturesMacro should generate correct GLSL code for a single texture", () => {
  ...
  expect(result, "should initialize sampledDiffuseColor").toContain("vec4 sampledDiffuseColor = diffuseColor");
  expect(result, "should include texture 0 processing comment").toContain("// Process texture 0");
  expect(result, "should include show condition").toContain("if (uShows[0] == 1)");
  expect(result, "should initialize texColor0").toContain("vec4 texColor0 = vec4(0.)");
  expect(result, "should calculate alpha").toContain("float alpha0 = texColor0.a * uOpacities[0]");
  expect(result, "should include alpha threshold check").toContain("if (alpha0 > 0.01)");
  expect(result, "should include mix operation").toContain("sampledDiffuseColor = mix(sampledDiffuseColor, vec4(texColor0.rgb, 1.0), alpha0)");
});
...
test("generateMixOverlaidTexturesMacro should generate correct GLSL code for two textures", () => {
  ...
  expect(result, "should initialize sampledDiffuseColor").toContain("vec4 sampledDiffuseColor = diffuseColor");
  expect(result, "should include texture 0 processing").toContain("// Process texture 0");
  expect(result, "should include texture 1 processing").toContain("// Process texture 1");
  expect(result, "should include show condition for texture 0").toContain("if (uShows[0] == 1)");
  expect(result, "should include show condition for texture 1").toContain("if (uShows[1] == 1)");
});
...
test("generateMixOverlaidTexturesMacro should handle multiple textures correctly", () => {
  ...
  expect(result, "should include texture 0 processing").toContain("// Process texture 0");
  expect(result, "should include texture 1 processing").toContain("// Process texture 1");
  expect(result, "should include texture 2 processing").toContain("// Process texture 2");
});
...
test("generateMixOverlaidTexturesMacro should use custom insert function when provided", () => {
  ...
  expect(result, "should include custom insert code").toContain("texture2D(uTextures[0], customUV)");
});
...
test("generateMixOverlaidTexturesMacro should handle the insert function as used in tile.ts", () => {
  ...
  expect(result, "should include the custom UV logic").toContain("vec2 texUv = vUv");
  expect(result, "should use the custom UV in texture2D").toContain("texture2D(uTextures[0], texUv)");
});
