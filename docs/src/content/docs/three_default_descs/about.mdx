...
`@navaramap/three-default-descs` is published on npm under the [`@navaramap`](https://www.npmjs.com/org/navaramap) scope. It requires `@navaramap/three`, `three`, and `postprocessing` as peer dependencies. See the [Install](../../three/introduction/install/) guide if they are not set up yet.
...
:::note
`@navaramap/three-default-plugin` depends on this package, so it is already present in `node_modules` when the plugin is installed. However, importing from `@navaramap/three-default-descs` in your own code (for example to reference Descriptor types such as `CloudsEffectDesc`) still requires the explicit install above, because transitive dependencies cannot be imported directly with some package managers (notably pnpm).
:::
