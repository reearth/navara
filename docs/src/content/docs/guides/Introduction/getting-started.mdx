...
Navara is published on npm under the [`@navaramap`](https://www.npmjs.com/org/navaramap) scope. In your project — any bundler setup works; a plain [Vite](https://vite.dev/) TypeScript project is enough — install the Three.js renderer `@navaramap/three`, the default plugin `@navaramap/three_default_plugin`, and the peer dependencies `three` and `postprocessing`:
...
Here is the core code — the same example shown in [What is Navara?](../what-is-navara/) — that creates a 3D globe, displays satellite imagery, and enables the photorealistic sky and atmosphere. Add it to your project's entry point (for example `src/main.ts` in a Vite project) and start the dev server:
