...
You need [Node.js](https://nodejs.org/) and a package manager such as npm, yarn, or pnpm installed on your machine.

## Project Setup

Any bundler setup works with Navara. This guide uses a [Vite](https://vite.dev/) TypeScript project. Scaffold one with the `vanilla-ts` template (see [Scaffolding Your First Vite Project](https://vite.dev/guide/#scaffolding-your-first-vite-project) for details):
...
Navara is published on npm under the [`@navaramap`](https://www.npmjs.com/org/navaramap) scope. In your project, install the Three.js renderer `@navaramap/three`, the default plugin `@navaramap/three-default-plugin`, and the peer dependencies `three` and `postprocessing`:
...
Here is the core code, the same example shown in [What is Navara?](../what-is-navara/). It creates a 3D globe, displays satellite imagery, and enables the photorealistic sky and atmosphere. Replace the contents of your project's entry point (`src/main.ts` in the Vite project above) with this code and start the dev server:
...
If you develop with an AI coding agent such as Claude Code, the Navara repository provides the [navara-usage skill](https://github.com/reearth/navara/tree/main/skills/navara-usage), a concise guide to best practices for `@navaramap/three`. It covers the setup order, common API mistakes, and working scene examples. Copy it into your project's skills directory (or point your agent at it) so the generated code follows correct API usage.
