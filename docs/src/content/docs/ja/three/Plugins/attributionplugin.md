...
`AttributionPlugin` は、地図のデータソースのクレジット UI を表示します。**`ThreeView` が既定で 1 つ生成し、`view.attribution` として公開します**（自前 UI を作る場合は `defaultAttribution: false` で無効化）。組込みの **「Navara」クレジットが常に先頭に表示される**ため、ソースを 1 つも追加していなくても UI は表示されます。ポップオーバーはアクティブなソースを一覧し、**既定では畳まれた状態**です。右下の小さな ⓘ トリガーで開閉でき（`hide()` / `show()` も同じ）、非モーダルなので開いたままでも地図は操作可能（パン / ズーム / 回転）です。
...
ユーザーが追加した出典をすべて削除しますが、プラグインは生かしたままにします（ポップオーバー・リスナー・注入したスタイルは残ります）。組込みの Navara クレジットと ⓘ トリガーは表示されたままで、ロゴが残っていなければロゴフレームだけが自動的に隠れます。DOM ごと破棄する場合は `dispose()` を使ってください。
...
出典ポップオーバーを開きます。既定では畳まれているため、開くにはこれを呼ぶ（または ⓘ トリガーを使う。どちらも同じ状態を切り替えます）必要があります。ポップオーバーのカードにのみ作用し、常時表示のロゴフレームはそのままです。
