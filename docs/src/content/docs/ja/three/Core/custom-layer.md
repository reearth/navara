...
class MyMeshDesc extends MeshDesc<
  Config,      // レイヤーの設定型（MeshConfig を拡張）
...
class MyEffectDesc extends EffectDesc<
  Config,      // レイヤーの設定型（EffectConfig を拡張）
...
class MyLightDesc extends LightDesc<
  Config,      // レイヤーの設定型（LightConfig を拡張）
...
`view.addMesh()` / `view.addEffect()` / `view.addLight()` から返される `BaseHandle<T>` は、レイヤーを制御するためのハンドルです。
