...
`this.view` を通じてシーンやカメラにアクセスできます。
...
| `view.renderPassOrchestrator`         | レンダーパイプラインの管理                        |
| `view.applyShadowMaterial(material)`  | CSM シャドウをマテリアルに適用（実験的）          |
| `view.removeShadowMaterial(material)` | CSM シャドウをマテリアルから削除（実験的）        |
