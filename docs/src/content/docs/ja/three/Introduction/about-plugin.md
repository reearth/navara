...
- **セットアップの自動化** — 開発者が実装したカスタムレイヤーを一括登録するプラグイン
...
three_default_layers（外部モジュール）
  └── Three.js 固有のメッシュ・エフェクト・ライトをレイヤーとして実装

three_default_plugin（外部モジュール）
  └── DefaultPlugin でデフォルトレイヤーの一括登録と高レベル API を提供
```

たとえば、[three_default_layers](../../../three_default_layers/about/) パッケージは Three.js 固有のメッシュやエフェクト、ライトを navara_three のレイヤー API を使用して実装しています。さらに [three_default_plugin](../../../three_default_plugin/about/) がこれらを一括で登録し、フォトリアルなシーンを簡単にセットアップできる高レベル API を提供します。
...
4. **利用** — 初期化後、プラグインが提供するメソッドやレイヤーを利用可能
...
[three_default_plugin](../../../three_default_plugin/about/) パッケージが提供する `DefaultPlugin` は、[three_default_layers](../../../three_default_layers/about/) の全 32 レイヤー（メッシュ 16 種、エフェクト 12 種、ライト 4 種）を一括登録するプラグインです。ほとんどのプロジェクトではこれだけで十分です。
...
- [three_default_layers](../../../three_default_layers/about/) - デフォルトレイヤーの実装
