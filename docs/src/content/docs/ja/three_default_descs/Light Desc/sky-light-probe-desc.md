...
`SkyLightProbeDesc`クラスは、大気散乱シミュレーションと連携した動的なスカイライトプローブDescriptorを表します。太陽の位置に応じて自動的に更新される環境照明を提供し、リアルな空の照明を再現します。
...
SkyLightProbeDescは以下の要素に基づいて自動的に更新されます：
...
:::note[更新可能なプロパティ]
Descriptor作成後に`update()`メソッドで変更できるプロパティは**`intensity`のみ**です。太陽の方向や位置などは自動的に更新されるため、手動で設定することはできません。
:::
...
SkyLightProbeDescは、`@takram/three-atmosphere`パッケージの大気散乱シミュレーションと密接に連携します：

1. **放射照度テクスチャの取得**: 大気オブジェクトから事前計算された放射照度テクスチャを取得
...
| 特徴         | SkyLightProbeDesc      | LightProbeDesc     |
| ------------ | ---------------------- | ------------------ |
...
- SkyLightProbeDescを使用するには、大気オブジェクト（Atmosphere）が必要です。
- `plugin.addDefaultPhotorealScene()`を使用すると、SkyLightProbeDescが自動的に含まれます。
- 夜間は強度を高めに設定することで、より自然な夜景照明を実現できます。
- 他のライト（AmbientLight、SunLightなど）と組み合わせて使用できます。
