...
`@navaramap/three-default-descs` は npm の [`@navaramap`](https://www.npmjs.com/org/navaramap) スコープで公開されています。peer dependencies として `@navaramap/three`・`three`・`postprocessing` が必要です。まだセットアップしていない場合は [Install](../../three/introduction/install/) ガイドを参照してください。
...
:::note
`@navaramap/three-default-plugin` はこのパッケージに依存しているため、プラグインをインストールすると `node_modules` には既に存在します。しかし、`CloudsEffectDesc` などのDescriptor型を参照する場合のように、アプリケーションのコードから `@navaramap/three-default-descs` を直接インポートするには、上記の明示的なインストールが必要です。パッケージマネージャーによっては（特に pnpm）、推移的依存のパッケージを直接インポートできないためです。
:::
