...
Navara は npm の [`@navaramap`](https://www.npmjs.com/org/navaramap) スコープで公開されています。プロジェクト（バンドラーは何でも構いません。素の [Vite](https://vite.dev/) TypeScript プロジェクトで十分です）に、Three.js レンダラーの `@navaramap/three`、デフォルトプラグインの `@navaramap/three_default_plugin`、および peer dependencies の `three` と `postprocessing` をインストールします。
