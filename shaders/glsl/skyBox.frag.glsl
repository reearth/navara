...
void main() {
    ...
    vec3 sunColor = mix(vec3(0.0), vec3(1.0), pow(sunInSightFactor, 14.0));
    vec3 dayColorFinal = sunColor + (uDayColor * 1.0 ) ;

    vec4 color = vec4(mix(uNightColor, dayColorFinal, smoothstep(0.4, 0.6, dayNightFactor)), 0.4);
    // vec4 color = vec4(mix(uNightColor, dayColorFinal, factor), 0.5);

    vec4 result = mix(color, vec4(0.0), 0.0);
