...
use super::{ParseMvtTileMarker, ParseMvtTileParameters};
use crate::{
...
pub(crate) fn parse_mvt_tile(
    ...
    constructors: Query<
        (Entity, &ParseMvtTileParameters, &OrderByDistance),
        ...
        .take(num_take)
    {
        let delegatee_id = commands
            .spawn(DelegatedWorkerTasksParametersBundle::new(
                ParseMvtTileMarker,
                DelegatedWorkerTasksParameters::ParseMvtTile(DelegatedWorkerTask::new(
                    e,
                    constructor.clone(),
                )),
                Priority::Medium,
