...
impl App {
    ...
    /// Load font bytes into the FontCache resource.
    /// Called from WASM when TypeScript fetches a font file.
    pub fn load_font(&mut self, url: String, data: Vec<u8>) -> Result<(), &'static str> {
        let Some(mut font_cache) = self
            .app
            .world_mut()
            .get_resource_mut::<navara_font::FontCache>()
        else {
            return Err("FontCache resource not found");
        };
        font_cache.insert(url, data)
    }

    /// Check if a font is loaded in the cache.
    ...
    /// Returns (pixel_data, width, height) or None if font isn't loaded.
    pub fn get_font_atlas(&self, url: &str) -> Option<(&[u8], u32, u32)> {
        let cache = self.app.world().get_resource::<navara_font::FontCache>()?;
        let entry = cache.get(url)?;
        Some((
            &entry.atlas.pixel_data,
            entry.atlas.width,
            entry.atlas.height,
        ))
    }
    ...
    pub fn get_units_per_em(&self, url: &str) -> Option<u16> {
        let cache = self.app.world().get_resource::<navara_font::FontCache>()?;
        let entry = cache.get(url)?;
        navara_font::shaping::get_units_per_em(&entry.data)
    }
    ...
    pub fn shape_text(
        ...
        navara_font::atlas::ensure_glyphs_in_atlas(
            &entry.sdf_font,
            &glyph_ids,
            &mut entry.atlas,
            current_frame,
