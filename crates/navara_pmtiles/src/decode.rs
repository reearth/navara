...
/// Zstd, and Unknown are rejected with [`PmtError::Decompression`] — they are
/// out of scope for the MVT-only first version.
///
/// # Errors
/// Returns [`PmtError::Decompression`] on an unsupported compression or a
/// corrupt gzip stream.
pub fn decompress(compression: Compression, data: &[u8]) -> Result<Vec<u8>, PmtError> {
    match compression {
        ...
        Compression::Gzip => {
            ...
            GzDecoder::new(data)
                .read_to_end(&mut out)
                .map_err(|_| PmtError::Decompression)?;
            Ok(out)
        }
        Compression::Brotli | Compression::Zstd | Compression::Unknown => {
            Err(PmtError::Decompression)
        }
    }
}
