...
pub fn transfer_batched_mesh(
    ...
    >,
    points: Query<(&TextGeometry, &BatchIndex)>,
...
pub fn transfer_mesh(
    ...
    for (entity, layer_id, batch_id, feature_id, geometry, material, lod_marker) in &mut texts {
        ...
        // Use RTC for all texts: transform contains absolute world position,
        // geometry contains relative coordinates (0, 0, 0 for single texts)
        let entity = commands
            .spawn((
                TextMarker,
                layer_id.clone(),
                RenderableFeature::Text {
                    ...
                    geometry: TransferablePointGeometry::with_buf_rte(
                        &mut buf,
                        vec![position.x, position.y, position.z], // RTC: relative offset is zero for single texts
                        vec![0],
                        vec![batch_id.0],
                    ),
...
pub fn update_height_by_terrain_for_batched(
    ...
    >,
    batched_features: Query<&BatchedFeature, (With<TextMarker>, Without<Deleted>)>,
            ...
            RenderableFeature::Text {
                ...
                for feature_id in &batched_feature.features {
                    ...
                    if material.clamp_to_ground {
                        // TODO: Avoid computing terrain height one by one. We can calculate it all together.
