...
use navara_quadtree::{QuadLeafHandle, decode_quadleaf_handle};
...
impl GeoJsonVt {
    ...
    pub fn key(&self, z: u32, x: u32, y: u32) -> TileKey {
        tile_key(x, y, z)
    }
    ...
    pub fn get_tile(&mut self, z: u32, x: u32, y: u32) -> Option<&Tile> {
        ...
        while z0 > 0 {
            ...
            x0 >>= 1;
            y0 >>= 1;
            let ancestor_key = self.key(z0, x0, y0);
    ...
    /// Checks if a tile exists at the given coordinates without drilling down.
    ///
    /// Only checks pre-indexed tiles (up to `index_max_zoom`).
    /// Use `get_tile` for on-demand drill-down to higher zoom levels.
    pub fn has_tile(&self, z: u32, x: u32, y: u32) -> bool {
        self.tiles.contains_key(&self.key(z, x, y))
    }
    ...
    /// Returns the number of pre-indexed tiles.
    pub fn tile_count(&self) -> usize {
        self.tiles.len()
    }

    /// Returns an iterator over all pre-indexed tiles.
